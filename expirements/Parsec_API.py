...
class prosecApi:
    ...
    # FindPersonByIdentifier = """<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Body><FindPersonByIdentifier xmlns="http://parsec.ru/Parsec3IntergationService"><sessionID>{}</sessionID><cardCode>{}</cardCode></FindPersonByIdentifier></soap:Body>"""#.format(sessionID, cardCode)
    ...
    def get_data_info_cardID(self, lastname='', firstname='', middlename=''):
        ...
        '''
        dict_res = self.get_info_user(lastname, firstname, middlename)
        Result = {}
        for key_people in dict_res:
            print(key_people)
            Result[key_people] = {}
            Result[key_people]['FIRST_NAME'] = dict_res[key_people]['FIRST_NAME']
            Result[key_people]['LAST_NAME'] = dict_res[key_people]['LAST_NAME']
            Result[key_people]['MIDDLE_NAME'] = dict_res[key_people]['MIDDLE_NAME']
            Result[key_people]['ORG_ID'] = dict_res[key_people]['ORG_ID']
    ...
    def open_turnstile(self, comand):
        '''
        ...
        '''
        self.__OpenSession()
        request = self.SendHardwareCommand.format(self.session, self.territoryId, comand)
        response = self.__pull_request(request)
...
if __name__ == '__main__':
    ...
    test.get_Person_Identifier('b9e1d2bc-ac4f-4b5b-bec3-fd586c8c3e49')